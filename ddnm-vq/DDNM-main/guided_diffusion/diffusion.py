...
from datasetss import get_dataset, data_transform, inverse_data_transform
...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if hasattr(self.config.model, 'type') and self.config.model.type == 'latent_compvis_ldm':
            ...
            return
        # in sample()
        if hasattr(self.config.model, 'type') and self.config.model.type == 'latent_celebahq_ldm':
            print('Run Latent DDNM (CelebA-HQ LDM UNet + VQ-VAE/SPNN codec).',
                  f'{self.config.time_travel.T_sampling} sampling steps. Task: {self.args.deg}.')
            self.latent_celebahq_ldm()
            return
    ...
    def latent_celebahq_ldm(self):
        args, config = self.args, self.config
        from functions.codec import load_codec_celebahq
        from functions.compvis_unet import load_celebahq_unet
    
        unet = load_celebahq_unet(self.device)
        vqvae_codec, spnn_codec = load_codec_celebahq(config, self.device)
        ...
        def model_fn(zt, t):
            ...
            if t.dim() == 0:
                t = t.unsqueeze(0).expand(zt.size(0))
            return unet(zt, t).sample  # unconditional, no text embeddings
    
        codecs_to_run = {"VQVAE": vqvae_codec}
        if spnn_codec is not None:
            codecs_to_run["SPNN"] = spnn_codec
        ...
        for codec_name, codec in codecs_to_run.items():
            print(f"\n--- Running with {codec_name} codec ---")
            self._codec = codec
            # CRITICAL: CelebA-HQ VQ latents are 3 channels at H/4, W/4
            self._latent_shape = (3, config.data.image_size // 4,
