...
def get_dataset(args, config):
    if getattr(config.data, "dataset", None) == "HF_CelebA_HQ":
        from datasetss.hf_celebahq import HFCelebAHQ
        ds = HFCelebAHQ(
            hf_name=getattr(config.data, "hf_name", "korexyz/celeba-hq-256x256"),
            split=getattr(config.data, "hf_split", "validation"),
            image_size=config.data.image_size,
        )
        return ds, ds  # (train, test) — DDNM only uses test
